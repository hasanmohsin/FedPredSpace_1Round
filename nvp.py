...
# File from https://github.com/ispamm/realnvp-demo-pytorch
...
class LinearRNVP(nn.Module):
    ...
    def __init__(self, input_dim, coupling_topology, flow_n=2, use_permutation=False,
                 batch_norm=False, mask_type='odds', 
                 conditioning_size=None, single_function=False,
                 task = "regression"):
        super().__init__()
    ...
    # given x, evaluate logp(x)
    # should be [B, ], B is batch size
    def get_logprob(self, x, y):
        u, log_det = self.forward(x, y=y) 

        # Train via maximum likelihood
        prior_logprob = self.logprob(u)
        log_prob = prior_logprob.sum(1) + log_det
...
def gen_train_step(gen, optimizer, trainloader, device):
        gen.train()
        ...
        for x, y in trainloader:
            ...
            if gen.task == "classification":
                y = torch.nn.functional.one_hot(y, gen.num_classes).to(device).float()
            else:
                y = y
            ...
            u, log_det = gen.forward(x, y=y) 

            # Train via maximum likelihood
            prior_logprob = gen.logprob(u)
            neg_log_prob = -torch.mean(prior_logprob.sum(1) + log_det)


            neg_log_prob.backward()
            optimizer.step()

            epoch_loss += neg_log_prob.item()

            count+=1
        return gen, epoch_loss
